...
subtitle: "Data visualisation with R - part 4"
author: "Hannah Meyer"
...
**Example**: select only names and location:
...
* first arument:  name of the `tibble` to which we want to add a variable
* subsequent argument:
    * name of the new variable
    * an equal sign `=` 
    * and the data to add. 

**Example**: reate a new variable, that contains the Euclidean
...
# Exercises
1. Find viruses that were isolated in the NETHERLANDS.
...
* `summaris`:  apply a summary statistic on this group. 
...
* use the R functions `min` and `max`, that find the minimum and maximum
  to find the start and end year, we can 
