...
with respect to their y-axis and legends alignment. As default, `plot_grid`
keeps the axis titles aligned. To align by axis, we can use a combination of
the `align` and `axis` arguments. Here we want the plots to be vertically
aligned along the left and right margins of the plot panel:
...
from the barplot with `theme(legend.position = "none")` should be removed.
...
All themes are `cowplot` themes apart from two `ggplot2` themes shown for
