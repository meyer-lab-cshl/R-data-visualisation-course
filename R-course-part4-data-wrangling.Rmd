...
by the names of the variables that we want to pick. For instance, we could
select only names and location:
```{r}
...
Below, we select all columns that end with the word 'coordinate':
```{r}
...
## Group analyses and summarise observations with `group_by` and `summarise()`
 ...
 cluster. To find the start and end year, we can use the R functions `min` and
...
as a representation of categorical variables with fixed possible values).
